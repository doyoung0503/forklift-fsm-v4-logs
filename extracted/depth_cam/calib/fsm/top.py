...
# - DETECTED: 최초 탐지의 팔레트 중심 방위각을 저장하고 정지
# - INITIAL_FACE_*: live PnP 중심 방위각으로 팔레트 중심을 바라보도록 회전
...
class CalibrationFSM:
    ...
    - INITIAL_FACE_*: live PnP 중심 방위각으로 팔레트 중심 방향 정렬
    ...
    def skip_detection(self) -> bool:
        """ALIGN의 타이머 기반 개방루프 동작 중에만 추론을 끈다.
    ...
    def step(self,
        ...
        if self.state == "SEARCH":
            ...
            if det_ok and target_bearing_deg is not None:
                ...
                self._ensure_stop()
                lines.append(("[SEARCH] 탐지는 됐지만 중심 키포인트 대기", COLOR_ALERT))
            else:
        ...
        if self.state == "DETECTED":
            ...
            lines.append(("[DETECTED→INITIAL_FACE_STOP] live PnP 유지", COLOR_META))
            return lines
        ...
        if self.state == "INITIAL_FACE_STOP":
            ...
            if target is None:
                self.state = "SEARCH"
                self._clear_initial_face()
                lines.append(("[INITIAL_FACE_STOP→SEARCH] 초기 중심 방위각 없음", COLOR_ALERT))
                return lines
        ...
        if self.state == "INITIAL_FACE_ROTATE":
            if not det_ok or target_bearing_deg is None:
                self.execu.exec("STOP")
                self.state = "SEARCH"
                self._clear_initial_face()
                lines.append(("[INITIAL_FACE_ROTATE→SEARCH] PnP 중심 방위각 유실", COLOR_ALERT))
                return lines

            error = self._wrap_to_180(float(target_bearing_deg))
            ...
            if reached:
                ...
                if self.tracer is not None:
                    self.tracer.step_end(final_center_bearing_deg=error)
                self.state = "INITIAL_FACE_SETTLE"
                self._initial_face_deadline = time.time() + INITIAL_FACE_SETTLE_SEC
                lines.append((f"[INITIAL_FACE_ROTATE→INITIAL_FACE_SETTLE] centre={error:+.2f}°", COLOR_META))
                return lines

            if time.time() >= self._initial_face_rotate_deadline:
                self.execu.exec("STOP")
                if self.tracer is not None:
                    self.tracer.step_end(timeout=True, final_center_bearing_deg=error)
                self.state = "SEARCH"
                self._clear_initial_face()
                lines.append((f"[INITIAL_FACE_ROTATE→SEARCH] timeout, centre={error:+.2f}°", COLOR_ALERT))
                return lines

            self.execu.exec(code)
            self.status.start_until(code, "|centre_bearing|", abs(error), YAW_TOL_DEG)
            lines.append((f"[INITIAL_FACE_ROTATE] PnP centre={error:+.2f}°", COLOR_STATUS_TRK))
            return lines
