...
class CommandExecutor:
    ...
    def __init__(self, tracer=None):
        ...
        # Debug-step mode can hold every motion command while still allowing
        # STOP. Normal operation leaves this enabled.
        self._movement_enabled: bool = True
    ...
    def set_movement_enabled(self, enabled: bool) -> None:
        """동작 명령을 허용하거나 즉시 STOP 상태로 유지한다."""
        enabled = bool(enabled)
        if enabled == self._movement_enabled:
            return
        self._movement_enabled = enabled
        if not enabled:
            # STOP은 동작 보류 중에도 항상 허용한다.
            self.exec("STOP")
    ...
    def exec(self, cmd: str) -> str:
        ...
        # 디버그 일시정지 시 새로 계획된 동작이 한 프레임이라도 차량에
        # 전달되지 않도록 차단한다. 다음 허용 동작은 STOP과 다르므로
        # 중복 억제에도 걸리지 않는다.
        if not self._movement_enabled and mapped_cmd != "STOP":
            if self._last_exec_cmd != "STOP":
                self.exec("STOP")
            return "STOP"
