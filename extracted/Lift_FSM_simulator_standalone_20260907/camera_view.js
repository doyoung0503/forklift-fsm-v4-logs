...
const LiftCameraView = (() => {
  ...
  async function pump() {
    ...
    let committed=false;
    const timing={},started=performance.now();
    try {
      let result=null,picture=null,jpeg=null,cameraError=null;
      try {
        result=await post('camera',{frame:job.frame,options:job.options});
        timing.camera_request_ms=performance.now()-started;
        timing.server_camera_ms=result.server_camera_ms;
        timing.renderer_ms=result.render_ms;
      ...
      } catch(error) {cameraError=error;timing.camera_error=error.message;picture=null;jpeg=null;}
      ...
      if(job.session) {
        ...
        timing.present_request_ms=performance.now()-presentStart;
        timing.server_present_ms=ack.server_present_ms;
        timing.present_lock_wait_ms=ack.server_lock_wait_ms;
      ...
      job.commit(job.frame,ack,timing);committed=true;
      timing.browser_paint_ms=performance.now()-paintStart;
      timing.display_total_ms=performance.now()-started;
    } catch(error) {
      if(epoch===generation) {
        status.hidden=false;
