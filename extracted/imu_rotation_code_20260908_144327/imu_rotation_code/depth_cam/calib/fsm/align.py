...
class AlignMachine:
    ...
    def __init__(self, execu: CommandExecutor, status: StatusHelper, tracer=None):
        ...
        self._rotation_target_deg: Optional[float] = None
    ...
    @property
    def is_imu_rotating(self) -> bool:
        """하위 호환용 IMU 회전 상태 플래그."""
        return self.sub in IMU_ROTATE_SUBS
    ...
    def reset(self, sub: str = "DIST_CHECK", clear_offset_done: bool = False) -> None:
        ...
        self._rotation_target_deg = None
    ...
    def _begin_rotation(self, sub: str, target_deg: float, rel_yaw: float) -> None:
        target_deg = _wrap_to_180(target_deg)
        self.sub = sub
        self._rotation_target_deg = target_deg
        self._rel_yaw_ref = float(rel_yaw)
        self._rotation_deadline_ts = time.time() + ALIGN_ROTATE_TIMEOUT_SEC
        code = "ROT_RIGHT" if target_deg > 0.0 else "ROT_LEFT"
        self._exec(code)
        self.status.start_until(code, "rel_yaw", 0.0, target_deg)

    def _rotation_step(
        self, rel_yaw: Optional[float], next_sub: str,
        lines: List[Tuple[str, tuple]],
    ) -> None:
        target, ref = self._rotation_target_deg, self._rel_yaw_ref
        if target is None or ref is None or rel_yaw is None:
            self._step_end(error="imu_unavailable")
            self._start_interlock_then("POST_LATERAL_INFER")
            lines.append((f"[{self.sub}] IMU 값 유실 -> 정지 후 재추론", COLOR_ALERT))
            return

        delta = _wrap_to_180(float(rel_yaw) - ref)
        ...
        if reached:
            ...
            self._step_end(actual_rel_yaw_deg=delta, target_rel_yaw_deg=target)
            self._rotation_target_deg = None
            self._rel_yaw_ref = None
            self._rotation_deadline_ts = None
            self._start_interlock_then(next_sub)
            lines.append((f"[{finished_sub}->{next_sub}] IMU {delta:+.2f}°", COLOR_META))
            return

        if self._rotation_deadline_ts is not None and time.time() >= self._rotation_deadline_ts:
            finished_sub = self.sub
            self._step_end(timeout=True, actual_rel_yaw_deg=delta, target_rel_yaw_deg=target)
            self._rotation_target_deg = None
            self._rel_yaw_ref = None
            self._rotation_deadline_ts = None
            self._start_interlock_then("POST_LATERAL_INFER")
        ...
        self._exec(code)
        lines.append((f"[{self.sub}] IMU {delta:+.2f}/{target:+.2f}°", COLOR_STATUS_TRK))
    ...
    def _apply_alignment_snapshot(
        self, yaw: float, ox: float, rel_yaw: float,
        lines: List[Tuple[str, tuple]],
    ) -> None:
        ...
        if abs(ox) > OFF_TOL_M:
            ...
            self._step_begin(
                ...
                driver="snapshot_yaw_plus_imu", snapshot_yaw_deg=yaw,
                offset_x_m=ox, target_rel_yaw_deg=target,
            )
            if abs(target) <= YAW_TOL_DEG:
                self._step_end(skipped=True, target_rel_yaw_deg=target)
                self._start_interlock_then("LATERAL_FORWARD")
            else:
                self._begin_rotation("LATERAL_PRE_ROTATE", target, rel_yaw)
            lines.append((
        ...
        if abs(yaw) > YAW_TOL_DEG:
            ...
            self._step_begin(
                ...
                driver="snapshot_yaw_then_imu", snapshot_yaw_deg=yaw,
                target_rel_yaw_deg=target,
            )
            self._begin_rotation("SNAPSHOT_YAW_ROTATE", target, rel_yaw)
    ...
    def step(
        ...
        if self.sub == "POST_DISTANCE_INFER":
            self._exec("STOP")
            if yaw is None or ox is None or rel_yaw is None:
                lines.append(("[POST_DISTANCE_INFER] yaw/lateral/IMU 결과 대기", COLOR_ALERT))
                return lines
            if dist_z is not None:
                self._latest_dist_z = float(dist_z)
            self._apply_alignment_snapshot(float(yaw), float(ox), float(rel_yaw), lines)
            return lines

        if self.sub == "SNAPSHOT_YAW_ROTATE":
            self._rotation_step(rel_yaw, "POST_LATERAL_INFER", lines)
            return lines

        if self.sub == "LATERAL_PRE_ROTATE":
            self._rotation_step(rel_yaw, "LATERAL_FORWARD", lines)
            return lines
        ...
        if self.sub == "LATERAL_FORWARD":
            ...
            self._motion_deadline_ts = None
            self._motion_code = None
            self._rotation_target_deg = -self._lateral_side * LATERAL_TURN_DEG
            self._rel_yaw_ref = None
            self._rotation_deadline_ts = None
        ...
        if self.sub == "LATERAL_FACE_ROTATE":
            if rel_yaw is None:
                self._exec("STOP")
                lines.append(("[LATERAL_FACE_ROTATE] IMU 값 대기", COLOR_ALERT))
                return lines
            if self._rel_yaw_ref is None:
                target = -self._lateral_side * LATERAL_TURN_DEG
                self._step_begin(
                    "lateral_face_rotate", "ROT_RIGHT" if target > 0.0 else "ROT_LEFT",
                    driver="imu_rel_yaw", target_rel_yaw_deg=target,
                )
                self._begin_rotation("LATERAL_FACE_ROTATE", target, float(rel_yaw))
            self._rotation_step(rel_yaw, "POST_LATERAL_INFER", lines)
            return lines
        ...
        if self.sub == "POST_LATERAL_INFER":
            ...
            self._apply_alignment_snapshot(stable_yaw, stable_ox, float(rel_yaw), lines)
            return lines
