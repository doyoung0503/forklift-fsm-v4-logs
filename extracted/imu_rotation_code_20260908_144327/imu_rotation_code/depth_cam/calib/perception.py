...
# 전체 키포인트는 다점 PnP에, 전면 4점은 시각화와 폴백에 사용한다.
...
class Perception:
    ...
    def infer_front(self, color_img) -> Tuple[bool, Optional[np.ndarray], Optional[tuple], Optional[np.ndarray]]:
        ...
        return: (det_ok, kpts4 (4,2) float32 [좌상,우상,우하,좌하] 픽셀좌표, bbox_xyxy or None,
                 kpts_all (K,3) float32 전체 키포인트[x,y,vis] or None — 다점 PnP/시각화용)
        """
        H, W = color_img.shape[:2]

        # FP16은 GPU에서만 유효. 현재 지게차 PC의 Ultralytics는 quantize 인자를
        # 지원하지 않으므로, deprecation 경고가 있더라도 호환되는 half 인자를 쓴다.
        ...
        # === 선택 기준 ===
        # PnP 전면부 실치수와 같은 종횡비를 사용해 후보를 고른다.
        ...
        def ar_distance(i: int) -> tuple:
            ...
            h = max(1.0, (y2 - y1))
            ar = w / h
            # 1순위: PnP 전면부 종횡비와의 차이 최소, 2순위: conf 큰 것(-confs[i])
        ...
        k = kpts[best_idx]
        kpts_all = k.astype(np.float32)   # (K,3) 전체 키포인트 — 다점 PnP와 화면 표시용
